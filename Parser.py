...
class Parser(object):
    ...
    def match(self, x):
        if self.token == x:
            ...
            self.flag = True
            #self.raise_error('Token Mismatch', self.token)
    ...
    def statement(self):
        if self.token == 'if':
            ...
            self.flag = True
            #self.raise_error('SyntaxError', self.token)
    ...
    def exp(self):
        t = self.simple_exp()
        if self.token == '<' or self.token == '=' or self.token == '>' or self.token == '<=' or self.token == '>=': 
            p = Node(
                'OP', '(' + self.token + ')', 'o')
            p.set_children(t)
            t = p
    ...
    def read_stmt(self):
        #Graph.add_node(1,label ='READ'+ '(' + self.code_list[self.tmp_index+1] + ')',shape='rectangle')
